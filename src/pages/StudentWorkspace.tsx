...
function iconForActivity(type: string): PixelIconName {
...
function ModalFrame({ children, onClose, label, wide = false }: { children: React.ReactNode; onClose: () => void; label: string; wide?: boolean }) {
  useEffect(() => {
  ...
  return (
    <div className="modal-layer" role="dialog" aria-modal="true" aria-label={label}>
      ...
      <section className={`${wide ? 'quest-modal wide-modal' : 'quest-modal'} panel`}>
        <button className="modal-close icon-button" onClick={onClose} aria-label={`Close ${label}`}><PixelIcon name="close" /></button>
        {children}
      </section>
    </div>
  );
...
function ProfileEditor({ profile, onClose, onSaved }: { profile: User; onClose: () => void; onSaved: (data: StudentDashboardData) => void }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    setError('');
    try {
      onSaved(await request<StudentDashboardData>('/api/profile', { method: 'PUT', body: JSON.stringify({ name, proficiency, learningGoal, dailyGoal }) }));
      onClose();
    } catch (err) {
...
function Onboarding({ profile, onComplete }: { profile: User; onComplete: (data: StudentDashboardData) => void }) {
  ...
  const submit = async () => {
    setBusy(true);
    try {
      onComplete(await request<StudentDashboardData>('/api/profile', { method: 'PUT', body: JSON.stringify({ name: profile.name, proficiency, learningGoal, dailyGoal }) }));
    } finally {
      setBusy(false);
    }
  };
...
function LessonDialog({ summary, onClose, onDashboard, notify }: { summary: LessonSummary; onClose: () => void; onDashboard: (data: StudentDashboardData) => void; notify: (message: string) => void }) {
  ...
  useEffect(() => {
    let mounted = true;
    request<Lesson>(`/api/lessons/${summary.id}`).then((loaded) => {
      if (!mounted) return;
      setLesson(loaded);
    ...
    }).catch((err) => { notify(err instanceof Error ? err.message : 'Could not open the lesson.'); onClose(); });
  ...
  const choose = (questionIndex: number, value: number | string) => {
    setAnswers((current) => {
      ...
      void request(`/api/lessons/${summary.id}/checkpoint`, { method: 'PUT', body: JSON.stringify({ lastQuestion: questionIndex, draftAnswers: next }) }).catch(() => undefined);
      return next;
    });
  };
  ...
  const saveStudy = async () => {
    try {
      await request(`/api/lessons/${summary.id}/study`, { method: 'PUT', body: JSON.stringify({ notes, bookmarked }) });
      notify('Study notes saved.');
    } catch (err) {
      notify(err instanceof Error ? err.message : 'Could not save notes.');
    }
  };
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const completed = await request<LessonResult>(`/api/lessons/${lesson.id}/complete`, { method: 'POST', body: JSON.stringify({ answers, durationSeconds: Math.round((Date.now() - startedAt.current) / 1000) }) });
      setResult(completed);
      ...
      notify(err instanceof Error ? err.message : 'Could not submit the lesson.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const practiseSpeaking = () => {
    ...
    recognition.onresult = (event) => {
      ...
      void request<{ transcript: string; accuracy: number; feedback: string }>(`/api/lessons/${lesson.id}/speaking-attempt`, { method: 'POST', body: JSON.stringify({ transcript }) })
        .then(setSpeechResult).catch((err) => notify(err instanceof Error ? err.message : 'Could not score the phrase.'));
    };
          ...
          <div className="answer-review">
            {result.review.map((item) => (
              <article className={item.correct ? 'correct' : 'incorrect'} key={item.prompt}>
                ...
                <div><strong>{item.prompt}</strong><small>Your answer: {item.type === 'fill_blank' ? String(item.selected) : lesson.questions.find((q) => q.prompt === item.prompt)?.choices[Number(item.selected)]}</small>{!item.correct && <small className="correct-answer">Correct answer: {item.type === 'fill_blank' ? String(item.answer) : lesson.questions.find((q) => q.prompt === item.prompt)?.choices[Number(item.answer)]}</small>}<small>{item.explanation}</small></div>
              </article>
            ))}
          </div>
          <div className="result-actions"><button className="secondary-button" onClick={() => { setResult(null); setAnswers(lesson.questions.map((q) => q.type === 'fill_blank' ? '' : -1)); startedAt.current = Date.now(); }}>Practise again</button><button className="primary-button" onClick={onClose}>Back to dashboard</button></div>
        </div>
      ) : (
          ...
          <div className="questions">
            {lesson.questions.map((question, questionIndex) => (
              <fieldset data-question-index={questionIndex} key={question.id || question.prompt}>
                ...
                {question.type === 'fill_blank' ? <input className="fill-answer" value={String(answers[questionIndex] ?? '')} onChange={(event) => choose(questionIndex, event.target.value)} placeholder="Type your answer" /> : question.choices.map((choice, choiceIndex) => (
...
function QuickQuizDialog({ onClose, onDashboard, notify }: { onClose: () => void; onDashboard: (data: StudentDashboardData) => void; notify: (message: string) => void }) {
  const [question, setQuestion] = useState<QuickQuestion | null>(null);
  const [choice, setChoice] = useState<number | null>(null);
  const [result, setResult] = useState<{ correct: boolean; answer: number; explanation: string; xpAwarded: number } | null>(null);
  const [busy, setBusy] = useState(false);
  const load = async () => {
    setChoice(null); setResult(null); setBusy(true);
    try { setQuestion(await request<QuickQuestion>('/api/quick-quiz')); }
    catch (err) { notify(err instanceof Error ? err.message : 'Could not load a quiz.'); onClose(); }
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const completed = await request<{ correct: boolean; answer: number; explanation: string; xpAwarded: number; dashboard: StudentDashboardData }>('/api/quick-quiz/submit', { method: 'POST', body: JSON.stringify({ questionId: question.id, answer: choice }) });
      setResult(completed); onDashboard(completed.dashboard); notify(completed.xpAwarded ? `Critical hit! +${completed.xpAwarded} XP` : completed.correct ? 'Correct! Daily XP already claimed.' : 'Knowledge gained—keep going.');
...
function VocabularyPanel({ items, onChanged, notify }: { items: VocabularyItem[]; onChanged: (items: VocabularyItem[]) => void; notify: (message: string) => void }) {
  const [term, setTerm] = useState('');
  const [definition, setDefinition] = useState('');
  const add = async (event: FormEvent) => {
    event.preventDefault();
    try {
      const item = await request<VocabularyItem>('/api/vocabulary', { method: 'POST', body: JSON.stringify({ term, definition }) });
      onChanged([item, ...items]); setTerm(''); setDefinition(''); notify('Word added to your study deck.');
    } catch (err) { notify(err instanceof Error ? err.message : 'Could not add the word.'); }
  };
  const remove = async (id: string) => {
    await request(`/api/vocabulary/${id}`, { method: 'DELETE' }); onChanged(items.filter((item) => item.id !== id));
  };
  return (
...
export function StudentWorkspace({ initialUser, onLogout }: { initialUser: User; onLogout: () => void }) {
  ...
  useEffect(() => {
    request<StudentDashboardData>('/api/dashboard').then(setData).catch((err) => setError(err instanceof Error ? err.message : 'Could not load the dashboard.'));
  }, []);
  ...
  const reset = async () => {
    ...
    try { setData(await request<StudentDashboardData>('/api/reset', { method: 'POST' })); notify('A fresh adventure begins.'); }
  ...
  return (
    <div className="app-shell student-shell">
      <header className="topbar">
        ...
        <button className="brand" onClick={() => scrollTo('dashboard')}><span className="brand-mark"><PixelIcon name="academy" size={23} /></span><span>English Pixel</span><i>Academy</i></button>
      ...
      <aside className={`side-drawer ${menuOpen ? 'open' : ''}`}><div className="drawer-head"><span className="brand"><span className="brand-mark"><PixelIcon name="academy" /></span><span>English Pixel</span><i>Academy</i></span><button className="icon-button" onClick={() => setMenuOpen(false)}><PixelIcon name="close" /></button></div><nav><button onClick={() => scrollTo('dashboard')}><PixelIcon name="home" /> Dashboard</button><button onClick={() => scrollTo('lessons')}><PixelIcon name="book" /> Courses & lessons</button><button onClick={() => scrollTo('assignments')}><PixelIcon name="scroll" /> Assignments</button><button onClick={() => scrollTo('study')}><PixelIcon name="brain" /> Study deck</button><button onClick={() => scrollTo('achievements')}><PixelIcon name="trophy" /> Achievements</button><button onClick={() => scrollTo('activity')}><PixelIcon name="magic" /> Activity</button></nav><div className="drawer-tip"><PixelIcon name="flame" size={34} /><strong>{data.profile.dailyGoal} minute daily goal</strong><small>{data.profile.learningGoal}</small></div><button className="drawer-close" onClick={onLogout}><PixelIcon name="logout" /> Sign out</button></aside>
