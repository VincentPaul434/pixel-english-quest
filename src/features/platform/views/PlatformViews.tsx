...
import {
  ...
  joinClassroom, leaveClassroom, postDiscussion, previewInvitation, removeClassroomStudent, requestVerification,
  resolveJoinRequest, revokeClassroomInvitation, setAdmin, setupMfa, submitWork, uploadAsset
} from '../models/api';
import type { AdminData, InvitationPreview, PlatformData, StudentLearningHubProps, TeacherOperationsProps } from '../models/types';
...
export function StudentLearningHub({ courses, assignments, notify }: StudentLearningHubProps) {
  ...
  useEffect(() => {
    ...
    void previewInvitation(inviteCode).then(setInvite).catch((error) => notify(error instanceof Error ? error.message : 'Could not open invitation.'));
  }, []);
  ...
  const inspectInvite = async (event: FormEvent) => {
    event.preventDefault();
    try { setInvite(await previewInvitation(inviteCode.trim())); }
    catch (error) { setInvite(null); notify(error instanceof Error ? error.message : 'Could not open invitation.'); }
  };
  const acceptInvite = async () => {
    if (!invite) return;
    try {
      setData(await joinClassroom(invite.code));
      setInvite(await previewInvitation(invite.code));
  ...
  const leave = async (classroomId: string) => {
    ...
    try { setData(await leaveClassroom(classroomId)); notify('You left the classroom.'); }
...
export function TeacherOperations({ courses, assignments, user, notify }: TeacherOperationsProps) {
  ...
  const decideRequest = async (requestId: string, status: 'accepted' | 'rejected') => { try { setData(await resolveJoinRequest(requestId, status)); notify(`Request ${status}.`); } catch (error) { notify(error instanceof Error ? error.message : 'Could not resolve request.'); } };
  const removeStudent = async (classroomId: string, studentId: string) => { if (!window.confirm('Remove this student and revoke classroom access?')) return; try { setData(await removeClassroomStudent(classroomId, studentId)); notify('Student removed.'); } catch (error) { notify(error instanceof Error ? error.message : 'Could not remove student.'); } };
  const revoke = async (invitationId: string) => { try { setData(await revokeClassroomInvitation(invitationId)); notify('Invitation revoked.'); } catch (error) { notify(error instanceof Error ? error.message : 'Could not revoke invitation.'); } };
